...
// 全选切换
    ...
    function up (obj) {
         ...
         var serchusername = $(obj).find('.serchusername').val();
         var onwork = $(obj).find('.onwork').val();
        num--;
        if (num<1) {
            num=1
        }
        $.post('<?=base_url();?>index.php/users/get_users',{"per_page":num,"department_id":department_id,'onwork':onwork,'serchusername':serchusername}, function(data) {
            var str;
            str='<tr>'+
                '<th></th>'+
                '<th>用户名</th>'+
    ...
    function down (obj) {
        ...
        var serchusername = $(obj).find('.serchusername').val();
         var onwork = $(obj).find('.onwork').val();
        $.post('<?=base_url();?>index.php/users/get_user_count',{"department_id":department_id,'onwork':onwork,'serchusername':serchusername}, function(pages) {
            var count = 0;
